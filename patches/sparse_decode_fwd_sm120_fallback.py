#!/usr/bin/env python3
"""
sparse_decode_fwd_sm120_fallback.py  (PATCH v3)
================================================

补丁：在 SM120 (Blackwell workstation / RTX PRO 5000) 上为 DeepSeek-V4-Flash
的 sparse MLA decode 路径加纯 PyTorch fallback，因为 FlashMLA 的
`flash_mla_cuda.sparse_decode_fwd` 是 SM100-only kernel。

V3 修复（vs v2）：
  v2 的 fallback 数学假设 Q.last_dim == 576 (V3 风格 NoPE_512+RoPE_64
  concat)，但 recon_q_shape.sh 的输出锁死了真相：
    * DeepseekV4SparseAttn._forward_decode 透传上层 q，没做 RoPE 拼接
    * 同源 flashmla_sparse.py:_fp8_flash_mla_kernel 也只 pad heads 维
      (q_padded[:, :, :H, :] = q)，不动 last dim
    * 实测 runtime 日志：q=(1, 1, 64, 512)
  → 真相 = 假设 1：Q 只送 NoPE-only latent (512 维)，K 是 [NoPE_448 |
    RoPE_64] = 512 维，attention 在 latent 空间逐位点积。
  V3 改动：
    1. 删除 Dq==576 断言，改为 Dq==512
    2. 删掉 `q_eff = cat(q[..., :448], q[..., 512:576])` 拼接逻辑
    3. 直接 Q(512) @ K(512).T 做 logits
    4. 文档更新真值

V2 修复（vs v1）：
  v1 dispatch block 用了 8 空格缩进，被注入到 `else: have_initialized`
  分支内部，导致第一次调用（走 `if` 建 config 分支）跳过 dispatch，
  fall-through 撞原始 sparse_decode_fwd。
  v2 改用 4 空格缩进，与 `if topk is not None:` 同级（函数体根缩进），
  无论第一次/后续调用都能拦截。
  同时 apply 后自动 ast.parse + clear __pycache__ + 缩进 self-check。

修改点：
  /usr/local/lib/python3.12/dist-packages/vllm/third_party/flashmla/flash_mla_interface.py
  在 `if topk is not None:` (函数体根缩进，line 448 附近) 之前注入 env 切换到纯 PyTorch 实现。
...
启动 vllm 命令（包含本补丁所需 env）：
    ...
    export VLLM_SPARSE_DECODE_FALLBACK=1          # 本补丁
    export VLLM_SPARSE_DECODE_FALLBACK_DEBUG=1    # 第一次 forward 打 1 行日志
...
用法（三件套）：
    python3 sparse_decode_fwd_sm120_fallback.py --check
    python3 sparse_decode_fwd_sm120_fallback.py --apply
    python3 sparse_decode_fwd_sm120_fallback.py --revert

数学/Layout 真值（已经通过 recon 锁定）：
  * KV cache 是 segregated 2D：
      [block_idx, kv_pos_in_block * TOKEN_STRIDE : ...]   ← fp8 NoPE + bf16 RoPE
      [block_idx, num_per_block * TOKEN_STRIDE + kv_pos_in_block * SCALE_DIM : ...] ← ue8m0 scales
    TOKEN_STRIDE = 576  (448B fp8 NoPE + 128B bf16 RoPE)
    SCALE_DIM    = 8    (7 个 ue8m0 + 1 个 pad)，每个 ue8m0 对应 64 NoPE 元素
    block 大小：SWA  64 tokens (584 = 576+8 字节段)；C4A/C128A 等大同
    解出得到 K = [NoPE_448 (fp8 dequant) | RoPE_64 (bf16 直读)] = 512 维 bf16
  * indices 含义：每个 entry = block_idx * block_size + offset_in_block
    （compute_global_topk_indices_and_lens 输出的就是 global token slot）
  * UE8M0 dequant：scale = 2 ** (byte_value - 127)
  * attn_sink 数学（line 90 文档原文）：
      out *= exp(lse) / (exp(lse) + exp(attn_sink))   # per head
      ≡    sigmoid(lse - attn_sink)
  * softmax 真值（V3 修正版）：
      # Q 由上层 attention layer 投影到 NoPE-only 512-维 latent 后传入；
      # 不像 V3 MLA 那样 concat RoPE。V4-Flash 的 sparse attention 完全
      # 在 latent 空间做。
      logits = (Q(512) @ K(512).T) * softmax_scale
      attn   = softmax(logits, dim=-1)
      out    = attn @ K(512)                    # V == K (latent attention)
